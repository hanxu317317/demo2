...
export default class Input extends Component
{
	...
	format = (value, country_code = this.state.country_code) =>
	{
		...
		let text = formatter.input(value);
		let template = formatter.template;
		let partially_populated_template = formatter.partially_populated_template;
		let country_phone_code = formatter.country_phone_code;
		let national_number = formatter.national_number;
		let national_prefix = formatter.national_prefix;
		let regExp = /[^0-9\+]/ig;
	...
	render()
	{
		...
		= this.props
		let errorStyle = error ? 'mcds-element__border' : '';
		const
		{
			value,
			country_code,
		...
		const markup =
		(
			<div style={ style } className={ classNames('react-phone-number-input', className, errorStyle) }>
